...
class Plan(BaseModel):
    substacks: list[str] = Field(..., description="問題を解決するためのサブタスクリスト")
...
class ReflectionResult(BaseModel):
    advice: str = Field(
        ...,
        description="""
評価がNGの場合は、別のツールを探す、別の文言で試すなど、なぜNGなのかどうしたら改善できるかを考えアドバイスを作成してください。
...
""",
    ...
    is_compiled: bool = Field(
        ...,
        description="ツールの実行結果と回答から、サブタスクに対して正しく回答できているかの評価。"
    )
...
class Subtask(BaseModel):
    ...
    substack_answer: str = Field(..., description="サブタスクの回答")
    challenge_count: int = Field(..., description="サブタスクの挑戦回数")

class AgentResult(BaseModel):
    question: str = Field(..., description="ユーザーのもとの質問")
